...
public final class LogNodeGen extends LogNode implements Provider {
    ...
    public Object execute(VirtualFrame frameValue) {
        int state = state_;
        if ((state & 0b10) == 0 /* only-active log(double) */ && (state & 0b11) != 0  /* is-not log(double) && log(Object) */) {
    ...
    private Object execute_double0(VirtualFrame frameValue, int state) {
        ...
        try {
            if ((state & 0b111000) == 0 /* only-active 0:double */ && (state & 0b11) != 0  /* is-not log(double) && log(Object) */) {
                arguments0Value_long = this.arguments0_.executeLong(frameValue);
                arguments0Value_ = JSTypes.longToDouble(arguments0Value_long);
            } else if ((state & 0b110100) == 0 /* only-active 0:double */ && (state & 0b11) != 0  /* is-not log(double) && log(Object) */) {
                arguments0Value_int = this.arguments0_.executeInt(frameValue);
                arguments0Value_ = JSTypes.intToDouble(arguments0Value_int);
            } else if ((state & 0b101100) == 0 /* only-active 0:double */ && (state & 0b11) != 0  /* is-not log(double) && log(Object) */) {
        ...
        assert (state & 0b1) != 0 /* is-active log(double) */;
        return LogNode.log(arguments0Value_);
    }

    private Object execute_generic1(VirtualFrame frameValue, int state) {
        Object arguments0Value_ = this.arguments0_.execute(frameValue);
        if ((state & 0b1) != 0 /* is-active log(double) */ && JSTypesGen.isImplicitDouble((state & 0b111100) >>> 2 /* extract-implicit-active 0:double */, arguments0Value_)) {
        ...
        if ((state & 0b10) != 0 /* is-active log(Object) */) {
            return log(arguments0Value_);
        }
    ...
    private double executeAndSpecialize(Object arguments0Value) {
        int state = state_;
        {
            ...
            if ((doubleCast0 = JSTypesGen.specializeImplicitDouble(arguments0Value)) != 0) {
                ...
                this.state_ = state = state | 0b1 /* add-active log(double) */;
                return LogNode.log(arguments0Value_);
            }
        }
        this.state_ = state = state | 0b10 /* add-active log(Object) */;
        return log(arguments0Value);
    }
    ...
    public Introspection getIntrospectionData() {
        ...
        s = new Object[3];
        s[0] = "log";
        if ((state & 0b1) != 0 /* is-active log(double) */) {
