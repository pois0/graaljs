...
public final class Log1pNodeGen extends Log1pNode implements Provider {
    ...
    public Object execute(VirtualFrame frameValue) {
        int state = state_;
        if ((state & 0b10) == 0 /* only-active log1p(double) */ && (state & 0b11) != 0  /* is-not log1p(double) && log1p(Object) */) {
    ...
    private Object execute_double0(VirtualFrame frameValue, int state) {
        ...
        try {
            if ((state & 0b111000) == 0 /* only-active 0:double */ && (state & 0b11) != 0  /* is-not log1p(double) && log1p(Object) */) {
                arguments0Value_long = this.arguments0_.executeLong(frameValue);
                arguments0Value_ = JSTypes.longToDouble(arguments0Value_long);
            } else if ((state & 0b110100) == 0 /* only-active 0:double */ && (state & 0b11) != 0  /* is-not log1p(double) && log1p(Object) */) {
                arguments0Value_int = this.arguments0_.executeInt(frameValue);
                arguments0Value_ = JSTypes.intToDouble(arguments0Value_int);
            } else if ((state & 0b101100) == 0 /* only-active 0:double */ && (state & 0b11) != 0  /* is-not log1p(double) && log1p(Object) */) {
        ...
        assert (state & 0b1) != 0 /* is-active log1p(double) */;
        return Log1pNode.log1p(arguments0Value_);
    }

    private Object execute_generic1(VirtualFrame frameValue, int state) {
        Object arguments0Value_ = this.arguments0_.execute(frameValue);
        if ((state & 0b1) != 0 /* is-active log1p(double) */ && JSTypesGen.isImplicitDouble((state & 0b111100) >>> 2 /* extract-implicit-active 0:double */, arguments0Value_)) {
        ...
        if ((state & 0b10) != 0 /* is-active log1p(Object) */) {
            return log1p(arguments0Value_);
        }
    ...
    private double executeAndSpecialize(Object arguments0Value) {
        int state = state_;
        {
            ...
            if ((doubleCast0 = JSTypesGen.specializeImplicitDouble(arguments0Value)) != 0) {
                ...
                this.state_ = state = state | 0b1 /* add-active log1p(double) */;
                return Log1pNode.log1p(arguments0Value_);
            }
        }
        this.state_ = state = state | 0b10 /* add-active log1p(Object) */;
        return log1p(arguments0Value);
    }
    ...
    public Introspection getIntrospectionData() {
        ...
        s = new Object[3];
        s[0] = "log1p";
        if ((state & 0b1) != 0 /* is-active log1p(double) */) {
